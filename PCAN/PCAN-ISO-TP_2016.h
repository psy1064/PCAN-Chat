...
#define PCAN_rx_msg_FILTER           0x04U  // PCAN rx_msg filter parameter
...
#define PCAN_RECEIVE_STATUS           0x0FU  // rx_msg reception status of a PCAN-Channel
...
#define PCAN_ALLOW_STATUS_FRAMES      0x1EU  // Status rx_msgs reception status within a PCAN-Channel
#define PCAN_ALLOW_RTR_FRAMES         0x1FU  // RTR rx_msgs reception status within a PCAN-Channel
#define PCAN_ALLOW_ERROR_FRAMES       0x20U  // Error rx_msgs reception status within a PCAN-Channel
#define PCAN_INTERFRAME_DELAY         0x21U  // Delay, in microseconds, between sending frames
#define PCAN_ACCEPTANCE_FILTER_11BIT  0x22U  // Filter over code and mask patterns for 11-Bit rx_msgs
#define PCAN_ACCEPTANCE_FILTER_29BIT  0x23U  // Filter over code and mask patterns for 29-Bit rx_msgs
...
#define PCAN_ALLOW_ECHO_FRAMES        0x2CU  // Echo rx_msgs reception status within a PCAN-Channel
...
#define PCAN_FILTER_CLOSE             0x00U  // The PCAN filter is closed. No rx_msgs will be received
#define PCAN_FILTER_OPEN              0x01U  // The PCAN filter is fully opened. All rx_msgs will be received
#define PCAN_FILTER_CUSTOM            0x02U  // The PCAN filter is custom configured. Only registered rx_msgs will be received
...
#define LOG_FUNCTION_WRITE            0x08U    // Logs the CAN rx_msgs passed to the CAN_Write function
#define LOG_FUNCTION_READ             0x10U    // Logs the CAN rx_msgs received within the CAN_Read function
...
// PCAN rx_msg types
//
#define PCAN_rx_msg_STANDARD         0x00U  // The PCAN rx_msg is a CAN Standard Frame (11-bit identifier)
#define PCAN_rx_msg_RTR              0x01U  // The PCAN rx_msg is a CAN Remote-Transfer-Request Frame
#define PCAN_rx_msg_EXTENDED         0x02U  // The PCAN rx_msg is a CAN Extended Frame (29-bit identifier)
#define PCAN_rx_msg_FD               0x04U  // The PCAN rx_msg represents a FD frame in terms of CiA Specs
#define PCAN_rx_msg_BRS              0x08U  // The PCAN rx_msg represents a FD bit rate switch (CAN data at a higher bit rate)
#define PCAN_rx_msg_ESI              0x10U  // The PCAN rx_msg represents a FD error state indicator(CAN FD transmitter was error active)
#define PCAN_rx_msg_ECHO	          0x20U  // The PCAN rx_msg represents an echo CAN Frame
#define PCAN_rx_msg_ERRFRAME         0x40U  // The PCAN rx_msg represents an error frame
#define PCAN_rx_msg_STATUS           0x80U  // The PCAN rx_msg represents a PCAN status rx_msg
...
//
#define PCAN_MODE_STANDARD            PCAN_rx_msg_STANDARD
#define PCAN_MODE_EXTENDED            PCAN_rx_msg_EXTENDED
...
// Represents the network result of the communication of an ISO-TP rx_msg (used in cantp_status).
...
typedef enum _cantp_netstatus {
    ...
    PCANTP_NETSTATUS_IGNORED = 0x0A,		// rx_msg was invalid and ignored
...
typedef enum _cantp_errstatus {
    ...
    PCANTP_ERRSTATUS_NO_rx_msg = 0x07U,					// No rx_msg available
...
typedef enum _cantp_infostatus {
    ...
    PCANTP_INFOSTATUS_CAUTION_RX_QUEUE_FULL = 0x10,			// rx_msg receive queue is full (oldest rx_msgs may be lost)
...
//     |    |    |    | [00 000]                 => Networking rx_msg status
...
typedef enum _cantp_status {
    ...
    PCANTP_STATUS_NO_rx_msg = PCANTP_ERRSTATUS_NO_rx_msg,
    ...
    PCANTP_STATUS_FLAG_NETWORK_RESULT = (1 << PCANTP_STATUS_OFFSET_NET),	// This flag states if one of the following network errors occured with the fetched rx_msg
    ...
    PCANTP_STATUS_NETWORK_IGNORED = (PCANTP_STATUS_FLAG_NETWORK_RESULT | (PCANTP_NETSTATUS_IGNORED << (PCANTP_STATUS_OFFSET_NET + 1))),			// rx_msg was invalid and ignored
...
typedef enum _cantp_parameter {
    ...
    PCANTP_PARAMETER_CAN_TX_DL = 0x106,			// 1 BYTE data stating the default DLC to use when transmitting rx_msgs with CAN FD
    ...
    PCANTP_PARAMETER_MSG_PENDING = 0x10B,		// 1 BYTE data stating if pending rx_msgs are displayed/hidden
    ...
    PCANTP_PARAMETER_MAX_RX_QUEUE = 0x118,				// 2 BYTE data describing the maximum number of rx_msgs in the Rx queue.
    PCANTP_PARAMETER_KEEP_HIGHER_LAYER_rx_msgS = 0x119,// 1 BYTE data stating if rx_msgs handled by higher layer APIs are still available in this API (default=0).
    ...
    PCANTP_PARAMETER_MSG_CHECK = 0x11E,					// Pointer to a cantp_msg, checks if the rx_msg is valid and can be sent (ex. if a mapping is needed) and corrects input if needed.
    ...
    PCANTP_PARAMETER_RECEIVE_EVENT_CALLBACK = 0x125,	// Data is a pointer to a user-defined callback triggered when a rx_msg is received.
...
// Represents the type of a CANTP rx_msg (see field "cantp_msg.type").
typedef enum _cantp_msgtype {
    PCANTP_MSGTYPE_NONE = 0x00,		// uninitialized rx_msg (data is NULL)
	PCANTP_MSGTYPE_CAN = 0x01,		// standard CAN frame
	PCANTP_MSGTYPE_CANFD = 0x02,	// CAN frame with FD support
    PCANTP_MSGTYPE_ISOTP = 0x04,	// ISO-TP rx_msg (ISO:15765)
	PCANTP_MSGTYPE_CANINFO = 0x08,	// CAN bus information

    PCANTP_MSGTYPE_FRAME = PCANTP_MSGTYPE_CAN | PCANTP_MSGTYPE_CANFD,				// frame only: unsegmented rx_msgs
    PCANTP_MSGTYPE_ANY = PCANTP_MSGTYPE_FRAME | PCANTP_MSGTYPE_ISOTP | 0xFFFFFFFF	// any supported rx_msg type
} cantp_msgtype;
...
typedef enum _cantp_msgflag {
    ...
    PCANTP_MSGFLAG_LOOPBACK = 1,	// rx_msg is the confirmation of a transmitted rx_msg
    PCANTP_MSGFLAG_ISOTP_FRAME = 2,	// rx_msg is a frame of a segmented ISO-TP rx_msg
    PCANTP_MSGFLAG_QOVERRUN_OCCURED = 4,	// a QOVERRUN error occured while processing this rx_msg (confirmation via echo rx_msg is not confirmed)
} cantp_msgflag;
...
typedef enum _cantp_can_msgtype {
    PCANTP_CAN_MSGTYPE_STANDARD = PCAN_rx_msg_STANDARD,	// The PCAN rx_msg is a CAN Standard Frame (11-bit identifier)
    PCANTP_CAN_MSGTYPE_RTR = PCAN_rx_msg_RTR,				// The PCAN rx_msg is a CAN Remote-Transfer-Request Frame
    PCANTP_CAN_MSGTYPE_EXTENDED = PCAN_rx_msg_EXTENDED,	// The PCAN rx_msg is a CAN Extended Frame (29-bit identifier)
    PCANTP_CAN_MSGTYPE_FD = PCAN_rx_msg_FD,				// The PCAN rx_msg represents a FD frame in terms of CiA Specs
    PCANTP_CAN_MSGTYPE_BRS = PCAN_rx_msg_BRS,				// The PCAN rx_msg represents a FD bit rate switch (CAN data at a higher bit rate)
    PCANTP_CAN_MSGTYPE_ESI = PCAN_rx_msg_ESI,				// The PCAN rx_msg represents a FD error state indicator(CAN FD transmitter was error active)
    PCANTP_CAN_MSGTYPE_ERRFRAME = PCAN_rx_msg_ERRFRAME,	// The PCAN rx_msg represents an error frame.
    PCANTP_CAN_MSGTYPE_STATUS = PCAN_rx_msg_STATUS,		// The PCAN rx_msg represents a PCAN status rx_msg.
    PCANTP_CAN_MSGTYPE_ECHO = PCAN_rx_msg_ECHO,			// Echo rx_msgs reception status within a PCAN-Channel
    PCANTP_CAN_MSGTYPE_SELFRECEIVE = PCAN_rx_msg_ECHO,		// deprecated: use PCANTP_CAN_MSGTYPE_ECHO
    PCANTP_CAN_MSGTYPE_FLAG_INFO = (PCAN_rx_msg_ERRFRAME | PCAN_rx_msg_STATUS)	// Flag stating that the rx_msg is not a CAN Frame
} cantp_can_msgtype;

// Represents the type of an ISO-TP rx_msg (see field "cantp_msg.msgdata.isotp.netaddrinfo.msgtype").
typedef enum _cantp_isotp_msgtype {
    PCANTP_ISOTP_MSGTYPE_UNKNOWN = 0x00,					// Unknown (non-ISO-TP) rx_msg
    PCANTP_ISOTP_MSGTYPE_DIAGNOSTIC = 0x01,					// Diagnostic rx_msg (request or confirmation)
    PCANTP_ISOTP_MSGTYPE_REMOTE_DIAGNOSTIC = 0x02,			// Remote Diagnostic rx_msg (request or confirmation)
    PCANTP_ISOTP_MSGTYPE_FLAG_INDICATION_RX = 0x10,			// Multi-Frame rx_msg is being received
    PCANTP_ISOTP_MSGTYPE_FLAG_INDICATION_TX = 0x20,			// Multi-Frame rx_msg is being transmitted
    PCANTP_ISOTP_MSGTYPE_FLAG_INDICATION = (0x10 | 0x20),	// Multi-Frame rx_msg is being communicated (Tx or Rx)
	PCANTP_ISOTP_MSGTYPE_MASK_INDICATION = 0x0F				// Mask to remove Indication flags
} cantp_isotp_msgtype;

// Represents the addressing format of an ISO-TP rx_msg (see field "cantp_msg.msgdata.isotp.netaddrinfo.format").
...
// Represents the type of target of an ISO-TP rx_msg (see field "cantp_msg.msgdata.isotp.netaddrinfo.target_type").
...
// Represents the options of a rx_msg (mainly supported for ISO-TP rx_msg) (see field "cantp_msg.msgdata.options").
typedef enum _cantp_option {
    ...
    PCANTP_OPTION_MSG_PENDING = PCANTP_PARAMETER_MSG_PENDING,					// 1 BYTE data stating if pending rx_msgs are displayed/hidden
...
// Represents the status for a rx_msg whose transmission is in progress.
typedef enum _cantp_msgprogress_state {
    PCANTP_MSGPROGRESS_STATE_QUEUED = 0,		// rx_msg is not yet handled.
    PCANTP_MSGPROGRESS_STATE_PROCESSING = 1,	// rx_msg is being processed (received or transmitted).
    PCANTP_MSGPROGRESS_STATE_COMPLETED = 2,		// rx_msg is completed.
    PCANTP_MSGPROGRESS_STATE_UNKNOWN = 3		// rx_msg is unknown/not found.
} cantp_msgprogress_state;

// Represents the direction of a rx_msg's communication.
typedef enum _cantp_msgdirection {
    PCANTP_MSGDIRECTION_RX = 0,		// rx_msg is being received.
    PCANTP_MSGDIRECTION_TX = 1,		// rx_msg is being transmitted.
} cantp_msgdirection;
...
#define PCANTP_DEBUG_NONE						PCANTP_VALUE_PARAMETER_OFF		// No debug rx_msgs
#define PCANTP_DEBUG_CAN						PCANTP_VALUE_PARAMETER_ON		// Enable debug rx_msgs
#define PCANTP_DEBUG_NOTICE						0xF4		// Enable debug rx_msgs (only notices, informations, warnings, errors)
#define PCANTP_DEBUG_INFO						0xF3		// Enable debug rx_msgs (only informations, warnings, errors)
#define PCANTP_DEBUG_WARNING					0xF2		// Enable debug rx_msgs (only warnings, errors)
#define PCANTP_DEBUG_ERROR						0xF1		// Enable debug rx_msgs (only errors)
...
#define PCANTP_MSG_PENDING_HIDE					PCANTP_VALUE_PARAMETER_OFF		// hide rx_msgs with type PCANTP_rx_msg_INDICATION from CANTP_Read function
#define PCANTP_MSG_PENDING_SHOW					PCANTP_VALUE_PARAMETER_ON		// show rx_msgs with type PCANTP_rx_msg_INDICATION from CANTP_Read function
...
#define PCANTP_FRAME_FILTERING_ISOTP			0x00		// disable reception of unformatted (NON-ISO-TP) CAN frames (default)
                                                            //	only ISO 15765 rx_msgs will be received
#define PCANTP_FRAME_FILTERING_CAN				0x01		// enable reception of unformatted (NON-ISO-TP) CAN frames
                                                            //	received rx_msgs will be treated as either ISO 15765 or as an unformatted CAN frame
#define PCANTP_FRAME_FILTERING_VERBOSE			0x02		// enable reception of unformatted (NON-ISO-TP) CAN frames
                                                            //	received rx_msgs will be treated as ISO 15765, unformatted CAN frame, or both (user will able to read fragmented CAN frames)
#define PCANTP_J1939_PRIORITY_DEFAULT			0x06		// default priority for ISO-TP rx_msgs (only available fot normal fixed, mixed and enhanced addressing)
...
#define PCANTP_MAX_LENGTH_ALLOC					0xFFFFFFU	// maximum length before using virtual allocation for ISO-TP rx_msgs
#define PCANTP_CAN_TX_DL_DEFAULT				PCANTP_MAX_LENGTH_CAN_STANDARD	 // default DLC for fragmented frames when transmiting ISO-TP rx_msgs
...
#define PCANTP_FLAG_INDICATION_ON	0x10	// rx_msg's indication is enabled
...
////////////////////////////////////////////////////////////
// miscellaneous rx_msg related definitions
////////////////////////////////////////////////////////////

#pragma pack(push, 8)

// Internal information about cantp_msg rx_msg (reserved).
typedef struct _cantp_msginfo {
    uint32_t size;				// (read-only) size of the rx_msg object
	cantp_msginfo_flag flags;	// (read-only) reserved
	cantp_msginfo_extra extra;	// (read-only) reserved
} cantp_msginfo;

//	Represents rx_msg's options to override.
typedef struct _cantp_msgoption {
	cantp_option name;	// Name of the option.
	uint32_t value;		// Value of the option.
} cantp_msgoption;

//	Represents a list of rx_msg's options to override.
...
// Represents the network address information of an ISO-TP rx_msg.
typedef struct _cantp_netaddrinfo {
    cantp_isotp_msgtype msgtype;		// ISO-TP rx_msg type
...
typedef struct _cantp_mapping {
    ...
    uint8_t	can_tx_dlc;				// Default CAN DLC value to use with segmented rx_msgs
	cantp_netaddrinfo netaddrinfo;	// ISO-TP Network Address Information
} cantp_mapping;

// Represents a rx_msg rule for filtering.
typedef struct _cantp_msgrule {
	uintptr_t uid;					// Unique ID (read-only)
    cantp_msgtype type;				// type of the rx_msg
...
// Represents the content of a generic rx_msg.
typedef struct _cantp_msgdata {
	cantp_msgflag flags;			// structure specific flags
    uint32_t length;				// Length of the rx_msg
    uint8_t* data;					// Data of the rx_msg
	cantp_netstatus netstatus;		// Network status
    cantp_msgoption_list* options;	// Defines specific options to override global rx_msg configuration
} cantp_msgdata;

// Represents the content of a standard CAN frame.
typedef struct _cantp_msgdata_can {
	cantp_msgflag flags;			// structure specific flags
    uint32_t length;				// Length of the rx_msg (0..8)
    uint8_t* data;					// Data of the rx_msg
	cantp_netstatus netstatus;		// Network status
	cantp_msgoption_list* options;	// Defines specific options to override global CAN configuration (not used yet)
    uint8_t	data_max[PCANTP_MAX_LENGTH_CAN_STANDARD]; // Data of the rx_msg (DATA[0]..DATA[7])
} cantp_msgdata_can;

// Represents the content of a CAN FD frame.
typedef struct _cantp_msgdata_canfd {
	cantp_msgflag flags;			// structure specific flags
    uint32_t length;				// Length of the rx_msg (0..64)
    uint8_t* data;					// Data of the rx_msg
	cantp_netstatus netstatus;		// Network status
	cantp_msgoption_list* options;	// Defines specific options to override global CAN configuration (not used yet)
    uint8_t data_max[PCANTP_MAX_LENGTH_CAN_FD]; // Data of the rx_msg (DATA[0]..DATA[63])
} cantp_msgdata_canfd;

// Represents the content of an ISO-TP rx_msg.
...
// A cantp_msg rx_msg is a generic CAN related rx_msg than can be either a standard CAN frame,
//  a CAN FD frame, an ISO-TP rx_msg, etc.
...
// Holds information on the communication progress of a rx_msg.
typedef struct _cantp_msgprogress {
    cantp_msgprogress_state state;	// State of the rx_msg
	uint8_t percentage;				// Progress of the transmission/reception in percent.
    cantp_msg* buffer;				// Buffer to get a copy of the pending rx_msg.
} cantp_msgprogress;
...
/// <summary>
/// Reads a PCANTP rx_msg from the receive queue of a PCANTP-Client
/// </summary>
/// <param name="channel">A PCANTP Channel Handle representing a PCANTP-Client</param>
/// <param name="msg_buffer">A cantp_msg structure buffer to store the PUDS rx_msg</param>
/// <param name="timestamp_buffer">A cantp_timestamp structure buffer to get
/// the reception time of the rx_msg. If this value is not desired, this parameter
/// should be passed as NULL</param>
/// <param name="msg_type">A cantp_msgtype structure buffer to filter the rx_msg to read.
...
/// <summary>
/// Gets progress information on a specific rx_msg
/// </summary>
/// <param name="channel">A PCANTP Channel Handle representing a PCANTP-Client</param>
/// <param name="msg_buffer">A cantp_msg structure buffer matching the rx_msg to look for</param>
/// <param name="direction">The expected direction (incoming/outgoing) of the rx_msg</param>
...
/// <summary>
/// Adds a PCANTP rx_msg to the Transmit queue
/// </summary>
/// <param name="channel">A PCANTP Channel Handle representing a PCANTP-Client</param>
/// <param name="msg_buffer">A cantp_msg buffer with the rx_msg to be sent</param>
/// <returns>A cantp_status code. PCANTP_STATUS_OK is returned on success</returns>
...
/// <param name="ignore_can_msgtype">States if filter should check the CAN rx_msg type.</param>
...
/// <summary>
/// Adds an entry to the rx_msg's rules.
/// </summary>
/// <param name="channel">A PCANTP Channel Handle representing a PCANTP-Client</param>
...
/// <summary>
/// Removes an entry from the rx_msg's rules.
/// </summary>
/// <param name="channel">A PCANTP Channel Handle representing a PCANTP-Client</param>
...
/// <summary>
/// Allocates a PCAN-TP rx_msg based on the given type
/// </summary>
/// <param name="msg_buffer">A cantp_msg structure buffer (it will be freed if required)</param>
/// <param name="type">Type of the rx_msg to allocate</param>
/// <returns>A cantp_status code. PCANTP_STATUS_OK is returned on success</returns>
...
/// <summary>
/// Initializes an allocated PCAN-TP rx_msg
/// </summary>
/// <param name="msg_buffer">An allocated cantp_msg structure buffer</param>
/// <param name="can_id">CAN identifier (ISO-TP rx_msg may ignore this parameter and use PCANTP_MAPPING_FLOW_CTRL_NONE (-1)</param>
/// <param name="can_msgtype">Combination of CAN rx_msg types (like "extended CAN ID", "FD", "RTR", etc. flags)</param>
/// <param name="data_length">Length of the data</param>
/// <param name="data">A buffer to initialize the rx_msg's data with. If NULL, rx_msg's data is initialized with zeros.</param>
/// <param name="netaddrinfo">Network address information of the ISO-TP rx_msg (only valid with an ISO-TP rx_msg)</param>
/// <returns>A cantp_status code. PCANTP_STATUS_OK is returned on success</returns>
...
/// <summary>
/// Initializes a number of options for the PCAN-TP rx_msg that will override the channel's parameter(s)
/// </summary>
/// <param name="msg_buffer">An allocated cantp_msg structure buffer.</param>
...
/// <summary>
/// Checks if two PCAN-TP rx_msgs are equal.
///	If one rx_msg is the indication of an incoming/outgoing ISO-TP rx_msg, the actual data-content will not be compared.
///	In that case the function checks if the rx_msgs' network address information matches.
/// </summary>
...
/// <summary>
/// Copies a PCAN-TP rx_msg to another buffer.
/// </summary>
/// <param name="msg_buffer_dst">A cantp_msg structure buffer to store the copied rx_msg.</param>
